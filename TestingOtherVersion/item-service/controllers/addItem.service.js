...
const addItem = async (req, res, database, bucket) => {
  ...
  const newItemRef = await database.collection(process.env.ITEMS_DOC).add({
    ownerId: req.user.id,
    itemName,
    itemDescription,
  ...
  if (photos && photos.length) {
    uploadedUrls = await uploadPhotos(newItemRef.id, photos, bucket);
