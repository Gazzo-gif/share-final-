...
const changePasswordWithLengthValidation = async (req, res) => {
  const { currentPassword, newPassword } = req.body;

  if (newPassword.length <= 6) {
    return res
      .status(httpStatus.BAD_REQUEST)
      .json({ error: 'Password needs to have atleast 6 characters' });
  }
...
module.exports = changePasswordWithLengthValidation;
